...
class CalendarEvntController {
	...
	def index() {
		respond response:responseSCService.gridResponse(CalendarEvnt.class,params)
	}

	@Transactional
	def save() {
		CalendarEvnt instance = CalendarEvnt.newInstance(params)
		...
		CalendarEvnt.withTransaction {status->
			try {
	...
	@Transactional
	def update() {
		CalendarEvnt instance
		try {
			instance = CalendarEvnt.lock(params.id)
		...
		if (!instance) {
			respond response:[status:-1,data:message(code:"default.not.found.message",args:[
					message(code:"calendarEvnt.label"),
					params.id
				])]
			return
		}

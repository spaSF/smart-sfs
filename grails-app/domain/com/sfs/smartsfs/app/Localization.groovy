...
class Localization implements Serializable {
	...
	static mapping = {
		table 'SC_LOCALIZATION'
		version true
		id generator:'native',params:[sequence:'SC_LOCALIZATION_SEQ']

		columns {
			code index: "sc_localizations_idx"
			locale column: "loc"
		}
	}
		...
		if (!msg) {
			Localization.withTransaction {
				def lst = Localization.findAll(
						"from com.sfs.smartsfs.app.Localization as x where x.code = ? and x.locale in ('*', ?, ?) order by x.relevance desc",
						[code, locale.getLanguage(), locale.getLanguage() + locale.getCountry()])
				msg = lst.size() > 0 ? (lst[0].text?:missingValue):missingValue
	...
	static load() {
		...
		if (path) {
			...
			if (grailsApplication.warDeployed) {
				grailsApplication.mainContext.getResources("**/WEB-INF/**/grails-app/i18n/**/*.properties")?.toList().each { messageFiles << it.file }
			} else {
				def i18nDirs = []
		...
		try {
			props.stringPropertyNames().each {key ->
				...
				if (!rec) {
					...
					txt = props.getProperty(key)
					if(txt!=null&&rec.text!=txt){
						log.debug "update localization ${loc} ${key} ${txt}"
						rec=Localization.lock(rec.id)
						rec.setText(txt)
						if (rec.validate()) {
							rec.save()
							counts.imported++
			...
			println "ERROR LOAD FILE ${file.name}"
		}
...
//	public void setUpdatedById(userId){

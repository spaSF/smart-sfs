...
class ${
	...
	def index() {
		respond response:responseSCService.gridResponse(${className}.class,params)
	}

	@Transactional
	def save() {
		${className} instance = ${className}.newInstance(params)
	...
	def update() {
		${className} instance
		try {
			instance = ${className}.lock(params.id)
